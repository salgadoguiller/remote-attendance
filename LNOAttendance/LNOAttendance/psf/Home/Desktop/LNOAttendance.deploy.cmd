...
set _DeploySetParametersFile=%RootPath%LNOAttendance.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%LNOAttendance.zip" (
echo "%RootPath%LNOAttendance.zip" does not exist. 
...
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%LNOAttendance.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension
) else (
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%LNOAttendance.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%_DeploySetParametersFile%"
)
...
:usage
echo =========================================================
if not exist "%RootPath%LNOAttendance.deploy-readme.txt" (
...
start notepad "%RootPath%LNOAttendance.deploy-readme.txt"
)
echo =========================================================
goto :eof
